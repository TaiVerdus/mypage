...
var CHAT_BACKEND = {
  ...
  system: [
    ...
    '· 数字分身的回答接的是**他自己的 DeepSeek API**（模型 deepseek-flash；key 只放在服务端的 .env 里，前端拿不到）；断网或额度不够时会落回本地预设问答，页面会标一行「离线版回答」（2026-09-26 他确认）',
    '',
    '必须遵守：',
...
var KNOWLEDGE = [
  ...
  /* 你接的是什么模型 / 你是真的 AI 吗（分身的大脑） */
  {
    keywords: ['什么模型','哪个模型','什么大模型','大模型','deepseek','api','离线版','接的是','接入'],
    answer: '分身接的是他自己的 DeepSeek API（模型 deepseek-flash），key 放在服务端、前端看不到。断网或额度不够时它会落回本地预设问答，那种回答页面会标一行「离线版回答」。'
  },

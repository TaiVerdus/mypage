...
var CHAT_BACKEND = {
  ...
  system: [
    ...
    '· 爱好：架子鼓、书法、音乐、摄影、篮球、旅游；平时在深圳探店。放松方式主要是听音乐',
...
var KNOWLEDGE = [
  /* 他在学什么 */
  {
    ...
    answer: '他这学期在上：微积分、线性代数、Python 编程，还有项目制课程。数学加编程——这是脑机接口方向的基本功。'
  },
  /* 他的方向 / 为什么选脑机接口 */
  {
  ...
  /* 他的爱好 / 平时怎么放松 */
  {
    keywords: ['interest','hobby','hobbies','drum','drums','calligraphy','basketball','fun','free time',
      'do for fun','爱好','兴趣爱好','玩什么','平时玩','喜欢什么','打鼓','书法','篮球','放松','怎么放松','放松方式'],
    answer: '他放松主要靠听音乐；课余还有架子鼓、书法、篮球、摄影这些。安静的吵闹的都有——打鼓和篮球是吵的，书法是静的。'
  },
  /* 他听什么音乐 */
  {
...
(function () {
  ...
  // ---------- 给「反馈专区」用的桥（2026-09-26）----------
  // 全站只在这里跟后台说话：客户端一个、错误翻译一份、镜像站判断一份、版本号一份。
  // 「反馈专区」（见文件末尾 交互十二）只准通过这扇门 —— 别在别处再建 client，
  // 否则同样的错会在两个地方翻译成两套话，版本号也会各写各的。
  window.MYPAGE_FB = {
    version: PAGE_VERSION,
    send: function (data, cb) { sendToBackend(data, cb); },
    // 读公开反馈墙：读的是 **feedback_public 视图**（只含「公开 + 已确认」的行，且只暴露 id/name/message/created_at）
    listPublic: function (cb) {
      if (isMirrorSite()) { cb('mirror'); return; }
      if (!backendReady()) { cb('down'); return; }
      var client;
      try { client = getCloud(); } catch (e) { cb('down'); return; }
      Promise.resolve(client.database.from('feedback_public')
        .select('name,message,created_at')
        .order('created_at', { ascending: false })
        .limit(24))
        .then(function (res) {
          if (res && res.error) cb('error');
          else cb(null, (res && res.data) || []);
        })
...
// ---------- 交互十二：反馈专区（V3.1，2026-09-26 用户要求）----------
// 页面上的「留言墙」：访客自己选这条给谁看。
//   · 私密 → 只有本人能看到（后台 RLS 挡着，页面连读都读不到）
//   · 公开 → 本人确认后才挂出来（读的是 feedback_public 视图，只含「公开 + 已确认」的行）
// ⚠️ 后台只准通过 MYPAGE_FB 这扇门（在下面的反馈抽屉模块里定义），别在这里另建 client。
(function () {
  var form = document.getElementById('fbzForm');
  if (!form || !window.MYPAGE_FB) return;

  var fieldsEl = document.getElementById('fbzFields');
  var nameEl   = document.getElementById('fbzName');
  var msgEl    = document.getElementById('fbzMsg');
  var countEl  = document.getElementById('fbzCount');
  var visEls   = form.querySelectorAll('input[name=fbzVis]');
  var errEl    = document.getElementById('fbzError');
  var doneEl   = document.getElementById('fbzDone');
  var doneTxt  = document.getElementById('fbzDoneText');
  var subm     = document.getElementById('fbzSubmit');
  var againBtn = document.getElementById('fbzAgain');
  var listEl   = document.getElementById('fbzList');
  var emptyEl  = document.getElementById('fbzEmpty');
  var noteEl   = document.getElementById('fbzWallNote');
  var nEl      = document.getElementById('fbzWallN');
  var MAX = 1000;
  var busy = false;
  ...
  function pickedVis() {
    for (var i = 0; i < visEls.length; i++) { if (visEls[i].checked) return visEls[i].value; }
    return 'private';
  }
  function syncRadio() {
    // :has() 现在主流浏览器都支持，这里再挂一个类名兜底（老浏览器也不至于看不出选了哪条）
    Array.prototype.forEach.call(visEls, function (r) {
      var box = r.closest ? r.closest('.fbz-radio') : null;
      if (box) { if (r.checked) box.classList.add('fbz-radio--on'); else box.classList.remove('fbz-radio--on'); }
    });
  }
  ...
  function showErr(t) { if (!errEl) return; errEl.textContent = t; errEl.hidden = false; }
  ...
  function itemEl(name, text, dateText, pending) {
    ...
    li.innerHTML = '<p class="fbz-item-head">' +
      ...
      '<p class="fbz-item-text">' + esc(text) + '</p>';
    return li;
  }
  ...
  function loadWall() {
    if (!listEl) return;
    window.MYPAGE_FB.listPublic(function (err, rows) {
      if (err === 'mirror') {
        if (nEl) nEl.textContent = '';
        listEl.innerHTML = '';
        if (noteEl) {
          noteEl.hidden = false;
          noteEl.textContent = '这是镜像站，读不到反馈墙（后台只接受正式地址与本机预览）。';
        }
        return;
      }
      if (err) {
        if (noteEl) { noteEl.hidden = false; noteEl.textContent = '反馈墙暂时没加载出来 —— 稍后刷新试试。'; }
        return;
      }
  ...
  if (msgEl) msgEl.addEventListener('input', function () { updateCount(); hideErr(); });
  Array.prototype.forEach.call(visEls, function (r) { r.addEventListener('change', syncRadio); });
  syncRadio();
  updateCount();
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (!msg) { showErr('还没写内容呢 —— 写两句就行。'); if (msgEl) msgEl.focus(); return; }
    if (msg.length > MAX) { showErr('有点长了，控制在 ' + MAX + ' 字以内。'); return; }
    hideErr();
    busy = true;
    if (subm) subm.setAttribute('aria-disabled', 'true');   // 边界用 aria-disabled，不用 disabled
    var vis = pickedVis();
    var who = (nameEl && nameEl.value ? nameEl.value.trim() : '');

    window.MYPAGE_FB.send({
      name: who,
      message: msg,
      visibility: vis,                 // 'public' | 'private' —— 后台按这个决定谁看得到
      version: window.MYPAGE_FB.version
    }, function (err) {
      busy = false;
      if (subm) subm.setAttribute('aria-disabled', 'false');
      if (err) { showErr('没发出去：' + err); return; }

...
| **公开** | 你确认后，所有访客 | 先以 `status='pending'` 进表；你在面板把它改成 `approved` 后才上墙 |
...
### 9.2 你要做的：审核公开反馈（几秒钟一条）

访客选了「公开」的那条，会带着 `status = pending` 躺在表里，**不会自动上墙**。让它上墙：
云服务面板 → 数据管理 → `feedback` 表 → 找到那行 → 把 `status` 改成 `approved` → 页面刷新即出现。
（想让某条从墙上撤下来：把 `status` 改回 `pending`，或把 `visibility` 改成 `private`。）
...
**2026-09-26 实测**：私密不可读 ✓ / 未审核不显示 ✓ / 审核后出现 ✓ / 基础表空数组 ✓ / 视图字段集正确 ✓。

### 9.4 已知限制

- **审核是手动的**：没有后台界面，靠你在云服务面板改 `status`（一条几秒；量大了再说自动化）；
